...
resource toast 'Microsoft.App/containerApps@2022-03-01' = {
  name: 'ca-toast-${resourceToken}'
  location: location
  tags: union(tags, {
      'azd-service-name': 'toast'
    })
  identity: {
    type: 'SystemAssigned'
  }
...
resource keyVaultAccessPolicies 'Microsoft.KeyVault/vaults/accessPolicies@2021-10-01' = {
  ...
  properties: {
    accessPolicies: [
      {
        objectId: toast.identity.principalId
        permissions: {
          secrets: [
            'get'
            'list'
...
output ORDERS_URI string = 'https://${toast.properties.configuration.ingress.fqdn}'
