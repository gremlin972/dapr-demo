...
resource delivery 'Microsoft.App/containerApps@2022-03-01' = {
  name: 'ca-delivery-${resourceToken}'
  location: location
  tags: union(tags, {
      'azd-service-name': 'delivery'
    })
  identity: {
    type: 'SystemAssigned'
  }
...
resource keyVaultAccessPolicies 'Microsoft.KeyVault/vaults/accessPolicies@2021-10-01' = {
  ...
  properties: {
    accessPolicies: [
      {
        objectId: delivery.identity.principalId
        permissions: {
          secrets: [
            'get'
            'list'
...
output ORDERS_URI string = 'https://${delivery.properties.configuration.ingress.fqdn}'
