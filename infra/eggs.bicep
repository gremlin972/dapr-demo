...
resource eggs 'Microsoft.App/containerApps@2022-03-01' = {
  name: 'ca-eggs-${resourceToken}'
  location: location
  tags: union(tags, {
      'azd-service-name': 'eggs'
    })
  identity: {
    type: 'SystemAssigned'
  }
...
resource keyVaultAccessPolicies 'Microsoft.KeyVault/vaults/accessPolicies@2021-10-01' = {
  ...
  properties: {
    accessPolicies: [
      {
        objectId: eggs.identity.principalId
        permissions: {
          secrets: [
            'get'
            'list'
...
output ORDERS_URI string = 'https://${eggs.properties.configuration.ingress.fqdn}'
