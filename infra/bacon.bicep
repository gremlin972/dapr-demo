...
resource bacon 'Microsoft.App/containerApps@2022-03-01' = {
  name: 'ca-bacon-${resourceToken}'
  location: location
  tags: union(tags, {
      'azd-service-name': 'bacon'
    })
  identity: {
    type: 'SystemAssigned'
  }
...
resource keyVaultAccessPolicies 'Microsoft.KeyVault/vaults/accessPolicies@2021-10-01' = {
  ...
  properties: {
    accessPolicies: [
      {
        objectId: bacon.identity.principalId
        permissions: {
          secrets: [
            'get'
            'list'
...
output ORDERS_URI string = 'https://${bacon.properties.configuration.ingress.fqdn}'
